    ...
    if request.method == 'POST':
        ...
        if address_form.is_valid() and u_form2.is_valid():
            ...
            if pay_mode == 'on':
                ...
                send_mail('order successful', 'Thanks for ordering', 'CoolLens@gmail.com', [request.user.email], fail_silently=False)
                ...
                if Orders.objects.all().last():
                    ...
                    order_id = 'ordr001'
                product1 = new_prod+'|'+str(1)+','
                request.session['new_prod']=new_prod
                request.session['order_id']=order_id
                request.session['product1']=product1
                request.session['prod_size']=prod_size
...
def contact(request):
    if request.method == 'POST':
        ...
        email_message = f"Name: {cont_name}\nEmail: {cont_email}\nSubject: {cont_subject}\nMessage: {cont_mess}"

        # Send the email using Gmail's SMTP server
        send_mail(
            'Contact Form Submission',
            email_message,
            cont_email,  # Sender's email address
            ['coollens@gmail.com'],  # Replace with your Gmail address or recipient's email address
            fail_silently=False,
        )
...
def stripe(request):
		if request.method == 'POST':
			new_prod=request.session.get('new_prod')
			order_id=request.session.get('order_id')
			product1=request.session.get('product1')
			prod_size=request.session.get('prod_size')
			try:
				Orders(order_id=order_id, user=request.user, saler=Product.objects.filter(product_id=int(new_prod)).first().shop, products=product1, size=prod_size).save()
			...
			return render(request,'main/stripe.html')
def stripe2(request):
    if request.method == 'POST':
        cart_prods = [p for p in Cart.objects.all() if p.user == request.user]
        trends = [i.product.product_id for i in trend.objects.all()]
        ...
        return render(request, 'main/stripe2.html')
